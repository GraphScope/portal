...
const SplitSection: React.FunctionComponent<ISplitSectionProps> = props => {
  const { styles, leftSide, rightSide, children, splitNumber = 0, smallScreen = false } = props;
  const colSize = smallScreen ? 24 : splitNumber;
  return (
    <>
      <Row style={{ padding: '2% 17%', ...styles }}>
        <Flex vertical gap={24}>
          <Flex align="center" gap={60}>
            <Col xs={colSize} sm={colSize} lg={colSize} xl={12}>
              {leftSide}
            </Col>
