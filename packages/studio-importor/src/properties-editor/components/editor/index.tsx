                    ...
                    {propertyConfigParams?.selectedRows.length == 0 ?
                        <Space>
                            <Popconfirm
                                ...
                                title={() => {
                                    return (
                                        <div style={{ width: '350px', padding: '0 12px' }}><span>Map from file</span>
                                            <Checkbox indeterminate={mapConfigParams?.selectedMapRowKeys.length > 0 && mapConfigParams?.selectedMapRowKeys.length < mapConfigParams?.dataSource.length}
                                                checked={mapConfigParams?.selectedMapRowKeys.length === mapConfigParams?.dataSource.length}
                                                onChange={mapConfigParams?.handleSelectAll} style={{ float: 'right' }}>
                                                Select all
                                            </Checkbox>
                                        </div>
                                    )
                                }}
                                description={<div style={{ width: '350px' }}><EditTable rowKey='mapfromfile' bordered={mapConfigParams?.bordered} showHeader={mapConfigParams?.showHeader} columns={mapConfigParams?.columns} dataSource={mapConfigParams?.dataSource} /></div>}
                                icon=''
                            ...
                            <Tooltip title="Add property">
                                <Button
                                    type="dashed"
                                    block
                                    onClick={()=>propertyConfigParams?.addNodeConfig()}
                                >
                                    <PlusOutlined />
                                </Button>
                            </Tooltip>
