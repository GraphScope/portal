...
if (jsFile && cssFile) {
  ...
  const inlineHtml = html
    .replace(/<script type="module" src="\.\/assets\/[^"]+"><\/script>/, `<script>${jsContent}</script>`)
    .replace(/<link rel="stylesheet" href="\.\/assets\/[^"]+">/, `<style>${cssContent}</style>`);
  
  // 保存内联版本
  const inlineHtmlPath = path.join(__dirname, 'dist', 'standalone.html');
  fs.writeFileSync(inlineHtmlPath, inlineHtml);
  
  console.log('Created standalone.html with inlined assets');
}

// 移除 type="module" 属性以兼容 file:// 协议
html = html.replace(/type="module"\s+/g, '');

// 写回原始文件
fs.writeFileSync(htmlPath, html);

// 检查并复制 vite.svg 如果不存在
...
console.log('Fixed crossorigin attributes and paths in index.html');
console.log('Removed type="module" for file:// compatibility');
console.log('Created vite.svg icon file');
