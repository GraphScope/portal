...
class DockerConfig {
  ...
  constructor() {
    ...
    try {
      ...
      logger.info(`Docker socket exists: ${stats.isSocket() ? 'Yes (is socket)' : 'No (not a socket)'}`);
    } catch (error: any) {
  ...
  private getDefaultDockerSocket(): string {
    ...
    try {
      ...
      const socketMatch = colimaStatus.match(/socket: unix:\/\/(.*docker\.sock)/);
    ...
    try {
      const dockerContext = execSync('docker context inspect 2>/dev/null').toString();
      ...
      if (hostMatch && hostMatch[1] && hostMatch[1].startsWith('unix://')) {
        ...
        logger.info(`Detected Docker socket from docker context: ${detectedPath}`);
        if (fs.existsSync(detectedPath)) {
          return detectedPath;
        }
      }
    } catch (error: any) {
      logger.warn(`Failed to detect Docker socket from context: ${error.message}`);
    }
