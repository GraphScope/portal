...
export function registerTerminalSocket(io: Server) {
  ...
  const sessions = new Map<string, { containerId: string; exec: Docker.Exec; stream: NodeJS.ReadableStream }>();

  io.on("connection", (socket: Socket) => {
    socket.on("start-terminal", async ({ containerId }: { containerId: string }) => {
      logger.info(`start-terminal: ${containerId}`);
      try {
        const container = containerService.getDockerInstance().getContainer(containerId);
        // 创建exec实例
        ...
        exec.start({ hijack: true, stdin: true }, (err, stream) => {
          if (err || !stream) {
            socket.emit("output", `Failed to start terminal: ${err?.message || "unknown error"}\r\n`);
            return;
          }
